...
storage.title("X-O-X - NANI")

tk.Label(storage, text="NANI", font=('ariel', 45, ), fg='red').pack()
tk.Label(storage, text="X-O-X", font=('Ariel', 25)).pack()
...
def check_win():
    for possibility in winning_possibilities:
        if possibility.check('X'):
            status_label.configure(text="Result: TEAM A won!")
            disable_game()
            return
        elif possibility.check('O'):
            status_label.configure(text="Result: TEAM B won!")
            disable_game()
            return
